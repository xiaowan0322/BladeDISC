...
def set_qconfig(ctx: GraphModContext) -> None:
    for node in ctx.quantizable_nodes():
        ...
        if not hasattr(m, 'qconfig') or ctx.is_override_qconfig:
...
def _fused_module(m: nn.Module) -> Tuple[nn.Module, Optional[nn.Module]]:
...
def insert_w_observer(ctx: GraphModContext) -> None:
    ...
    for node in ctx.quantizable_nodes():
        m, fused_module = _fused_module(ctx.modules[node.target])
...
def quantizable_module_to_observed(ctx: GraphModContext) -> None:
    ...
    """
    for node in ctx.quantizable_nodes():
        src, fused_module = _fused_module(ctx.modules[node.target])
